...
use cronus_spec::{
    GolangGinGeneratorOption, PythonFastApiGeneratorOption, RawSchema, RawUsecaseMethod, RawUsecaseMethodRestOption
};
use std::{any::type_name, cell::RefCell, collections::HashSet, fmt::format, path::PathBuf};

use crate::{
    utils::{
        self, get_path_from_optional_parent, get_request_name, get_response_name, get_schema_by_name, get_usecase_name, spec_ty_to_golang_builtin_ty, spec_ty_to_py_builtin_ty, spec_ty_to_rust_builtin_ty
    },
    Ctxt, Generator,
};
...
impl Generator for GolangGinGenerator {
    ...
    fn before_all(&self, ctx: &Ctxt) -> Result<()> {
        ...
        let mut imports_str = imports.iter()
    ...
    fn generate_usecase(
        ...
        let has_rest_methods = usecase.methods.iter().any(|(_, method)| {
            method.option.as_ref().and_then(|option| option.rest.as_ref()).is_some()
        });
        ...
        for (method_name, method) in &usecase.methods {
            ...
            result += &format!("  {}.{}(\"{}\", {})\n", 
            rg_var, rest.method.to_ascii_uppercase(), 
            rest_path, 
...
impl GolangGinGenerator {
    ...
    fn gen_usecase_method(
        ...
        if let Some(req) = &method.req {
            let request_ty = get_request_name(ctx, method_name);
            result += &format!("  var request {}\n", self.gen_anonymous_struct(ctx, req, Some(request_ty.clone())));
        ...
        if let Some(extra_request_fields) = self.get_gen_option(ctx)
            .as_ref()
        ...
        let res_var = if has_res {
            "response, "
        } else {
            ""
        };
        result += &format!("  {} err := {}.{}(ctx, &domain_request)\n", res_var, usecase_var,method_name.to_case(Case::UpperCamel));
        result += "  if err != nil {\n";
