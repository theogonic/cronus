...
import { Inject, Controller, Get, Post, Delete, Put, Param, Query, Body, ParseIntPipe, ParseBoolPipe, ParseArrayPipe, ArgumentMetadata, HttpStatus, Injectable, Optional, ParseIntPipeOptions, PipeTransform, ParseBoolPipeOptions } from "@nestjs/common";
import { HttpErrorByCode } from "@nestjs/common/utils/http-error-by-code.util";
import { ZeusParseIntPipe, ZeusParseBoolPipe } from "./zeusHelpers";
import { GeneralObjectStatus, GeneralObjectMeta, AddressType, TypedAddress, PhoneType, TypedPhone, JustTest, Pagination, User, UserChange, UserService, USER_SERVICE, GetUserResponse, GetUserRequest, ListUsersResponse, ListUsersRequest } from "./types";
...
export class UserCreateEventBodyDto {
    @ApiPropertyOptional()
    name?: string;
}
...
export class UserServiceController {
    ...
    userId: string, 
    @User()
    invoker): Promise<GetUserResponseDto> {
    ...
    @Query("name")
    name: string, 
    @Query("p_totalCount", new ZeusParseIntPipe({
        optional: true
    }))
    p_totalCount: number, 
    @Query("p_nextToken")
    ...
    p_t_type: string, 
    @User()
    invoker): Promise<ListUsersResponseDto> {
